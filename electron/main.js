...
function setupIpcHandlers() {
  ...
  ipcMain.handle(
    ...
    async (event) => {
      console.log(
        "🔄 Check for updates called, isDev:",
        isDev
      );
      ...
      if (!isDev) {
        try {
          console.log(
            "📦 Production mode - trying auto-updater"
          );

          if (!autoUpdater) {
            console.error(
              "❌ Auto-updater not available in production"
            );
            // Graceful fallback - direct user to GitHub
            ...
            setTimeout(() => {
              event.sender.send(
                ...
                {
                  message:
                    "Auto-update not available. Please check GitHub for updates.",
                  url: "https://github.com/quochoatqh/try-to-publish/releases/latest",
                }
              );
            }, 1000);
            return;
          ...
          console.log(
            "✅ AutoUpdater ready, checking for updates..."
          );
          ...
          setTimeout(() => {
            event.sender.send(
              ...
              {
                message:
                  "Auto-update failed. Please check GitHub manually.",
                url: "https://github.com/quochoatqh/try-to-publish/releases/latest",
              }
            );
          }, 1000);
        ...
        console.log(
          "🛠️ Development mode - simulating update check"
        );
        ...
        setTimeout(() => {
          console.log(
            "📝 Sending simulated update-not-available"
          );
          event.sender.send(
            "update-not-available",
            { version: "1.0.5" }
...
async function setupAutoUpdater() {
  try {
    if (!autoUpdater) {
      console.error(
        "❌ Failed to get autoUpdater instance"
      );
      return null;
    }
